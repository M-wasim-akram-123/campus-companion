...
export const Route = createFileRoute("/api/staff/by-role")({
  server: {
    handlers: {
      GET: async ({ request }) => {
        try {
          ...
          return json({ error: error instanceof Error ? error.message : "Failed to load staff by role." }, 500);
        }
      },
    },
  },
