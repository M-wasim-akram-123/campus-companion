-- Run after patch-fee-module.sql (copy of migrations/20260525120000_finance_module.sql)
...
CREATE POLICY "Authenticated fee vouchers" ON public.fee_vouchers FOR ALL TO authenticated USING (true) WITH CHECK (true);
DROP POLICY IF EXISTS "Authenticated fee voucher lines" ON public.fee_voucher_lines;
CREATE POLICY "Authenticated fee voucher lines" ON public.fee_voucher_lines FOR ALL TO authenticated USING (true) WITH CHECK (true);
DROP POLICY IF EXISTS "Authenticated fee payments" ON public.fee_payments;
CREATE POLICY "Authenticated fee payments" ON public.fee_payments FOR ALL TO authenticated USING (true) WITH CHECK (true);
DROP POLICY IF EXISTS "Authenticated fee payment allocations" ON public.fee_payment_allocations;
CREATE POLICY "Authenticated fee payment allocations" ON public.fee_payment_allocations FOR ALL TO authenticated USING (true) WITH CHECK (true);
DROP POLICY IF EXISTS "Authenticated finance counters" ON public.finance_counters;
CREATE POLICY "Authenticated finance counters" ON public.finance_counters FOR ALL TO authenticated USING (true) WITH CHECK (true);
...
BEGIN
  INSERT INTO public.finance_counters (counter_key, year, last_number) VALUES (p_key, y, 1)
