-- Run in Supabase SQL Editor if the project already has the inquiries table.
...
DROP POLICY IF EXISTS "Staff view inquiry interactions" ON public.inquiry_interactions;
CREATE POLICY "Staff view inquiry interactions" ON public.inquiry_interactions
  FOR SELECT TO authenticated
  USING (public.is_staff(auth.uid()));

DROP POLICY IF EXISTS "Staff insert inquiry interactions" ON public.inquiry_interactions;
CREATE POLICY "Staff insert inquiry interactions" ON public.inquiry_interactions
  FOR INSERT TO authenticated
  WITH CHECK (public.is_staff(auth.uid()));

DROP POLICY IF EXISTS "Super admin delete inquiry interactions" ON public.inquiry_interactions;
