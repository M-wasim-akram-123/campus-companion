...
-- Root cause: student_finance_ledger has ON DELETE CASCADE from students, but
-- trg_prevent_finance_ledger_update blocks DELETE/UPDATE. Deleting a student
-- therefore fails with: "Finance ledger entries are immutable..."
--
-- Fix: allow mutations only when session flag app.allow_student_purge = 'on',
-- set inside SECURITY DEFINER admin_purge_student().
...
BEGIN
  ...
  -- Unlink converted inquiry so it returns to the pipeline.
  ...
  -- Permit cascade/SET NULL against immutable finance tables for this transaction only.
